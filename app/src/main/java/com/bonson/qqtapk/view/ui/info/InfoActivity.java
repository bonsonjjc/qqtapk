...
public class InfoActivity extends BaseDaggerActivity {
  ...
  @Inject InputFragment inputFragment;
  ...
  public class CityAdapter implements CityPickerDialog.CityAdapter {
    @Override public int provinceSize() {
      return province.length;
    }

    @Override public int citySize(int index) {
      return city.length;
    }

    @Override public int districtSize(int index) {
      return district.length;
    }

    @Override public String province(int index) {
      if (index == 0) return "--";
      return province[index - 1];
    }

    @Override public String city(int index) {
      if (index == 0) return "--";
      return city[index - 1];
    }

    @Override public String district(int index) {
      if (index == 0) return "--";
