...
public class WalkingRouteOverlay extends OverlayManager {
  ...
  @Override public final List<OverlayOptions> getOverlayOptions() {
    if (mRouteLine == null) {
      return null;
    }
    ...
    if (mRouteLine.getAllStep() != null && mRouteLine.getAllStep().size() > 0) {
      ...
      for (WalkingRouteLine.WalkingStep step : mRouteLine.getAllStep()) {
        ...
        if (watPoints != null) {
          ...
          overlayList.add(new PolylineOptions().points(points)
              .width(10)
  ...
  @Override public final boolean onMarkerClick(Marker marker) {
